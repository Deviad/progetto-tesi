...
public interface ReactiveReadRepository<T, ID> extends Repository<T, ID> {
   ...
   * @return {@link Mono} emitting the entity with the given id or {@link Mono#empty()} if none
   *     found.
   ...
   *
   * @param id must not be {@literal null}. Uses the first emitted element to perform the
   *     find-query.
   * @return {@link Mono} emitting the entity with the given id or {@link Mono#empty()} if none
   *     found.
   ...
   *
   * @param id must not be {@literal null}.
   * @return {@link Mono} emitting {@literal true} if an entity with the given id exists, {@literal
   *     false} otherwise.
   * @throws IllegalArgumentException in case the given {@literal id} is {@literal null}.
   */
  ...
  /**
   * Returns whether an entity with the given id, supplied by a {@link Publisher}, exists. Uses the
   * first emitted element to perform the exists-query.
   *
   * @param id must not be {@literal null}.
   * @return {@link Mono} emitting {@literal true} if an entity with the given id exists, {@literal
   *     false} otherwise.
   ...
   * @return {@link Flux} emitting the found entities. The size can be equal or less than the number
   ...
   * @throws IllegalArgumentException in case the given {@link Publisher idStream} is {@literal
   *     null}.
   */
