...
public class UserEntity {
  ...
  public Set<UUID> addCourse(UUID courseId) {
    if(role == null) {
      throw new RuntimeException("You must be either a teacher or a student");
    }

    if(role.name().equals(Role.STUDENT.name())) {
      throw new RuntimeException("Only teachers can add new courses");
    }
  ...
  public Set<UUID> removeCourse(UUID courseId) {
    ...
    if(role.name().equals(Role.STUDENT.name())) {
      throw new RuntimeException("Only teachers can add remove courses");
    }
  ...
  public Set<UUID> enrollInCourse(UUID courseId) {
    ...
    if(role.name().equals(Role.TEACHER.name())) {
      throw new RuntimeException("You must be a student to enroll in a course");
    }
    courseIds.add(courseId);
    return courseIds;
  }
  ...
  public Set<UUID> unenrollFromCourse(UUID courseId) {
    ...
    if(role.name().equals(Role.TEACHER.name())) {
      throw new RuntimeException("You must be a student to unenroll from a course");
    }
    courseIds.remove(courseId);
    return courseIds;
  }
